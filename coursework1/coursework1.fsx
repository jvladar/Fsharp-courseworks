...
//   fst (notFibonacci n) = fst (notFibonacci (n - 2)) + fst (notFibonacci (n - 1))
//    
// 
// and
// 
...
let rec sinApprox (d:int) (n:float) : float =
    match d with
        | 0 -> n
        | _ -> 2.0 * (sinApprox (d-1) (n/2.0)) * (cosApprox (d-1) (n/2.0))
